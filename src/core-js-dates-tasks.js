...
function getDayName(date) {
  ...
  return weekArray[varDate.getUTCDay()];
}
...
function getNextFriday(date) {
  const varDate = new Date(date);
  const dayOfTheWeek = varDate.getDay();

  let daysToAdd;
  if (dayOfTheWeek < 5) {
    daysToAdd = 5 - dayOfTheWeek;
  } else {
    daysToAdd = 7 - (dayOfTheWeek - 5);
  }

  varDate.setDate(varDate.getDate() + daysToAdd);
  return varDate;
}
...
function getCountDaysInMonth(month, year) {
  const varDate = new Date(year, month - 1);
  const nextMonth = new Date(year, month);
  const diffrenceInMs = Date.parse(nextMonth) - Date.parse(varDate);
  const msInOneDay = 1000 * 60 * 60 * 24;
  return diffrenceInMs / msInOneDay;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const diffrenceInMs = Date.parse(end) - Date.parse(start);
  const msInOneDay = 1000 * 60 * 60 * 24;
  return diffrenceInMs / msInOneDay + 1;
}
...
function formatDate(date) {
  const varDate = new Date(date);
  const day = varDate.getUTCDate();
  const month = varDate.getMonth() + 1;
  const year = varDate.getFullYear();
  let hour = varDate.getUTCHours();
  const minutes = String(varDate.getMinutes()).padStart(2, '0');
  const seconds = String(varDate.getSeconds()).padStart(2, '0');
...
function getCountWeekendsInMonth(month, year) {
  const varDate = new Date(year, month - 1);
  let result = 0;
  while (varDate.getMonth() === month - 1) {
...
function getWeekNumberByDate(date) {
  ...
  varDate.setDate(varDate.getDate() + 4 - (varDate.getDay() || 7));
  const newYear = new Date(varDate.getFullYear(), 0, 1);
  const diff = Date.parse(varDate) - Date.parse(newYear);
  const msInOneDay = 1000 * 60 * 60 * 24;
  const days = diff / msInOneDay + 1;
...
function getQuarter(date) {
  const varDate = new Date(date);
  const months = varDate.getUTCMonth();
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const result = [];
  const parsedStart = Date.parse(period.start.split('-').reverse().join('-'));
  const parsedEnd = Date.parse(period.end.split('-').reverse().join('-'));
  const startDate = new Date(parsedStart);
  const endDate = new Date(parsedEnd);
  let counter = 1;
  const workingShift = countWorkDays + countOffDays;
  while (endDate >= startDate) {
    if (counter <= countWorkDays) {
      result.push(
        `${String(startDate.getDate()).padStart(2, '0')}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${String(startDate.getFullYear()).padStart(2, '0')}`
      );
      counter += 1;
    } else {
      counter += 1;
    }
    if (counter > workingShift) {
      counter = 1;
    }
